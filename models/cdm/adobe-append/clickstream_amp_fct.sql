...
--create table `ncau-data-newsquery-dev.cdm_adobe.clickstream_amp_fct` 
...
(SELECT 
  'ADOBE AMP' AS SourceCode,post_visid_high, post_visid_low, visit_num, visit_page_num, post_prop2, post_prop3, post_prop4, post_prop5
	, post_prop6, post_prop9, page_url, post_prop12, post_prop14, event_list, post_prop17
...
-----------------------
  FROM {{ source ('sdm_adobe_amp','clickstream_ingest') }} A1
    WHERE
...
adobe_raw_records_02 as (
  ...
  *
  ...
  WHERE DATE(IngestTime) between "2021-11-01" and "2021-11-30"  ----- '2021-11-01' and '2021-11-30'  
),
adobe_raw_records_03_temp as (

    SELECT  *, article_access_type.ArticleAccessType post_prop15
    FROM adobe_raw_records_02 left join `cdm_content.capi_article_access` article_access_type on  (adobe_raw_records_02.post_prop16 = article_access_type.articleid  
                    and adobe_raw_records_02.date_time between article_access_type.startdate and article_access_type.enddate )
...
from  adobe_raw_records_03_temp
)
select *,
