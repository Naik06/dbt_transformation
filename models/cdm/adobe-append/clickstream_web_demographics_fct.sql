...
--create or replace table `ncau-data-newsquery-dev.cdm_adobe.clickstream_web_demographics_fct`
-- PARTITION BY
...
select web_subscriber_events.* except(page_view) ,
if (web_subscriber_events.articleid is not null and web_subscriber_events.contenttype not like '%acq+shopfront%'   and web_subscriber_events.contenttype
not like '%breach+shopfront%' and web_subscriber_events.contenttype not like '%shopfront%', page_view , 0) as page_view,
  ifnull(if(individual_gender is not null and individual_gender in ('m','f'), upper(individual_gender),individual_gender),'Not Available') as individual_gender,
  ifnull(lifestage_desc,'Unknown') lifestage_desc,
  ifnull(household_income_desc,'Unknown') household_income_desc,
  ifnull(substr(mosaic_type,1,1),'Unknown') mosaic_type,
  ifnull(individual_age,'Not Available') individual_age,
  ifnull(affluence_desc,'Unknown') affluence_desc
from {{ ref ('clickstream_web_fct') }} web_subscriber_events left join subscriber_demographics
on (web_subscriber_events.CONSUMER_PROFILE_ID = subscriber_demographics.dw_subscriber_party_id)
where dw_partition_date between "2021-11-01" and "2022-01-31" ---"2021-11-01" and "2022-01-31"
    ...
    and MemberType = 'Subscriber'
    UNION ALL 
  select web_non_subscriber_events.* except(page_view) ,
if (web_non_subscriber_events.articleid is not null and web_non_subscriber_events.contenttype not like '%acq+shopfront%'   and web_non_subscriber_events.contenttype
not like '%breach+shopfront%' and web_non_subscriber_events.contenttype not like '%shopfront%', page_view , 0) as page_view,
  ...
  ifnull(individual_age,'Not Available') individual_age,
  ifnull(affluence_desc,'Unknown') affluence_desc
from {{ ref ('clickstream_web_fct') }} web_non_subscriber_events left join anonymous_demographics
on ( web_non_subscriber_events.ADOBE_UUID = anonymous_demographics.dw_browser_id)
where dw_partition_date between "2021-11-01" and "2022-01-31" ---"2021-11-01" and "2022-01-31"
    --and date(datetime(date_time,"Australia/Sydney")) between @startDate and @endDate -----"2021-11-01" and "2022-01-31"
    and MemberType != 'Subscriber'
    --)